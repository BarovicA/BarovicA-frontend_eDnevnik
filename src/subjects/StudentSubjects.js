...
import ShowStudentSubjects from "./ShowStudentSubjects";  
import {
  Container,
  Grid,
  Box,
...
const StudentSubjects = () => {
  ...
  const fetchData = async () => {
    setLoading(true);
    const url = "http://localhost:8080/api/v1/students/mySubjectsWithMarks"; 
          ...
          <Grid container spacing={3}>
            {subjects.map((subject, index) => ( 
              <Grid item xs={12} sm={6} md={4} lg={3} key={index}>
                ...
                >
                  <ShowStudentSubjects subject={subject} /> 
                </AnimatedBox>
              </Grid>
            ))}
          </Grid>
